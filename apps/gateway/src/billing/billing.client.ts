...
import type {
  GetInvoicePayload,
  GetInvoiceRpcResponse,
  ListInvoicesPayload,
  ListInvoicesRpcResponse,
  RpcResponse,
} from "@apptit/contracts";
import { BILLING_GET_INVOICE, BILLING_LIST_INVOICES } from "@apptit/contracts";
...
export class BillingClientService {
  ...
  async listInvoices(
    payload: ListInvoicesPayload,
  ): Promise<ListInvoicesRpcResponse> {
    return this.send<ListInvoicesRpcResponse>(BILLING_LIST_INVOICES, payload);
  }

  async getInvoice(
    payload: GetInvoicePayload,
  ): Promise<GetInvoiceRpcResponse> {
    return this.send<GetInvoiceRpcResponse>(BILLING_GET_INVOICE, payload);
  }
